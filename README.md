...
| `get_whatsapp_profile`          | The profile on a WhatsApp number and how the platform rates it   |
| `update_whatsapp_profile`       | Update the profile fields the platform allows                    |
| `list_whatsapp_templates`       | Message templates with the review status the platform assigned   |
| `get_whatsapp_template_library` | The platform's pre-written templates, for adapting               |
| `create_whatsapp_template`      | File a template for review, from scratch or a library entry      |
| `list_whatsapp_flows`           | In-chat forms, with their status and validation errors           |
| `get_whatsapp_flow_responses`   | What people submitted through this account's flows               |
| `list_whatsapp_groups`          | Groups this WhatsApp number created, with their invite links     |
| `get_whatsapp_account_state`    | WhatsApp account review state and the number's standing          |
